...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		if((exit_status = pthread_mutex_init(&entry->lock, NULL)) != 0 ) {
			exit(exit_status);
		}
                             ...
                             uint32_t value)
{
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head;
	if ( (exit_status = pthread_mutex_lock(&hash_table_entry->lock)) != 0 ) {
		exit(exit_status);
	} 
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
	...
	if (list_entry != NULL) {
		...
		if ( (exit_status = pthread_mutex_unlock(&hash_table_entry->lock)) != 0 ) {
			exit(exit_status);
		} 
		return;
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
		if ( (exit_status = pthread_mutex_destroy(&entry->lock)) != 0 ) {
			exit(exit_status);
		} 
